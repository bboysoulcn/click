...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const slugs = Array.from(clickButtons).map(
      (button) => (button.dataset.slug || window.location.href) + '-' + encodeURIComponent(button.dataset.icon || 'heart')
    );
    ...
    const initializeButtons = async () => {
      ...
      clickButtons.forEach((button, index) => {
        ...
        const slug = (button.dataset.slug || window.location.href) + '-' + encodeURIComponent(button.dataset.icon || 'heart');
